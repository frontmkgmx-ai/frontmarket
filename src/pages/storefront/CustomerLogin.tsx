...
export function CustomerLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Após o login, redireciona para o checkout ou página inicial da loja
    ...
    <div className="min-h-[70vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-900">Entrar na {store.name}</h2>
          <p className="mt-2 text-sm text-gray-600">
            Não tem uma conta?{' '}
            <Link to={`/${store.slug}/register`} className="font-medium text-indigo-600 hover:text-indigo-500">
              Cadastre-se
            </Link>
          </p>
        </div>
