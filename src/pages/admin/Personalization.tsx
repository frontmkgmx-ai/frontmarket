...
const PDP_STYLES = [
  { id: 'default', name: 'Padrão' },
  { id: 'minimalist', name: 'Minimalista' },
  { id: 'gallery', name: 'Galeria Vertical' },
  { id: 'modern', name: 'Moderno (Cards)' },
  { id: 'classic', name: 'Clássico' },
  { id: 'fullwidth', name: 'Largura Total' },
  { id: 'centered', name: 'Centralizado' },
  { id: 'compact', name: 'Compacto' },
  ...
  useEffect(() => {
    if (activeStore?.settings) {
      setFormData({
        ...
        pdpStyle: activeStore.settings.pdpStyle || 'default'
      });
    }
  }, [activeStore]);
  ...
  const handleSaveLayout = async (e: React.FormEvent) => {
    ...
    try {
      await updateDoc(doc(db, 'stores', activeStore.id), {
        ...
        'settings.pdpStyle': formData.pdpStyle
      });
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Configure o tema, logotipo, estilos de cabeçalho, rodapé, loading e página de produto.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          {activeTab === 'layout' && (
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2 border-b border-slate-100 pb-2">
                ...
                <h3 className="text-lg font-bold text-slate-800">Página do Produto (PDP)</h3>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                {PDP_STYLES.map(style => (
                  <button
                    key={style.id}
                    type="button"
                    onClick={() => setFormData({ ...formData, pdpStyle: style.id })}
                    className={`p-3 text-left rounded-xl border-2 transition-all text-sm font-medium flex flex-col gap-1 ${
                      formData.pdpStyle === style.id 
                        ? 'border-indigo-600 bg-indigo-50 text-indigo-900' 
                        : 'border-slate-200 hover:border-indigo-200 bg-white text-slate-700'
                    }`}
                  >
                    <span className="block">{style.name}</span>
                    {formData.pdpStyle === style.id && <span className="text-[10px] uppercase font-bold text-indigo-500">Selecionado</span>}
                  </button>
                ))}
              </div>
            </div>
        ...
        <div className="w-full lg:w-96 shrink-0 lg:sticky lg:top-24 mt-8 lg:mt-0">
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col h-full min-h-[600px]">
            <h3 className="font-bold text-slate-800 mb-4 flex items-center justify-between">
              Pré-visualização
              <span className="px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded-full text-[10px] uppercase tracking-wider">Ao Vivo</span>
            </h3>
            <div className="flex-1 bg-slate-100 rounded-xl overflow-hidden">
              <StorefrontPreview 
                storeName={activeStore?.name || 'Minha Loja'}
                themeId={activeTab === 'themes' ? currentThemeId : (activeStore?.settings?.theme || 'default')}
                logoUrl={formData.logoUrl}
                headerStyle={formData.headerStyle}
                footerStyle={formData.footerStyle}
                loadingStyle={formData.loadingStyle}
                pdpStyle={formData.pdpStyle}
              />
            </div>
          </div>
        </div>
